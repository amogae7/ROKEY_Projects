...
class RobotController(Node):
    def __init__(self):
        ...
        self.get_logger().info("🤖 Robot Control Final v2 Ready (Pass/Non-pass Only)")
        self.init_thread()
    ...
    def show_bad_part_callback(self, msg):
        # [수정] Part 1, 2, 3 명령은 무시하고, Pass/Non-pass만 수행
        if "part" in msg.data:
            self.get_logger().info(f"🚫 Ignoring Show Command for {msg.data} (Parts are handled by another node)")
            return
    ...
    def dispose_bad_part_callback(self, msg):
        # [수정] Part 1, 2, 3 명령은 무시하고, Pass/Non-pass만 수행
        if "part" in msg.data:
            self.get_logger().info(f"🚫 Ignoring Dispose Command for {msg.data} (Parts are handled by another node)")
            return
    ...
    def execute_move(self, msg, action="show"):
        ...
        if action == "show":
            ...
            # 여기서 "pass"나 "non_pass"는 이 조건문에 걸려 전용 위치로 이동합니다.
            if "pass" in target_name or "non_pass" in target_name:
                ...
                # 이 코드는 실행될 일이 없지만(위에서 필터링됨) 안전장치로 둠
