...
class ExitActions:
    ...
    def pickup_car(self, car_type: str):
        ...
        if self.busy:
            self.node.get_logger().warn("[픽업] 이미 출차 시퀀스 실행 중 → 새로운 요청 무시")
            return
    ...
    def _pickup_done(self, future):
        ...
        self.node.create_timer(0.5, self._play_beep_and_exit_to_goal)

    def _play_beep_and_exit_to_goal(self):
        self.beep_node.play_beep()
        self.node.get_logger().info("[픽업] → exit_to_goal() 실행")
        self.node.create_timer(0.5)
        self.exit_to_goal()
