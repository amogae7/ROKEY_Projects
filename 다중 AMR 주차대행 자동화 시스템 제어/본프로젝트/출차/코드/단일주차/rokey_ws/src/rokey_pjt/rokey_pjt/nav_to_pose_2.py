...
class JsonNavToPoseWithPre(Node):
    ...
    def mark_task_assigned(self, task_id: str):
        ...
        try:
            ...
            self.get_logger().info(f":데이터베이스: Task 할당 완료: {task_id}")
    ...
    # ========================================
    def exit_ready_callback(self, msg: String):
        """
    ...
    def task_callback(self, msg: String):
        ...
        if not self.ready_for_task:
            self.get_logger().warn(
                "[task_callback] 현재 EXIT 작업 진행 중 → /robot5/exit_ready "
                "수신 전까지 새 Task 무시"
            )
            return
